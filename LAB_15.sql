...
-- 2.
CREATE TRIGGER PERSONINFO_TRIGGER_SALARY
ON PERSON_INFO
AFTER UPDATE
AS
BEGIN
	...
	UPDATE PERSON_INFO SET @SALARY = @SALARY / 1.1
END


-- PART C ::
-- 1.
CREATE TRIGGER PERSONINFO_TRIGGER_JOININGDATE
ON PERSON_INFO
AFTER INSERT
AS
BEGIN
	DECLARE @JOININGDATE DATE
	SELECT @JOININGDATE =  P_JOINING_DATE FROM INSERTED
	IF (@JOININGDATE IS NULL)
	BEGIN
		SET @JOININGDATE = GETDATE()
		UPDATE PERSONINFO SET P_JOINING_DATE = @JOININGDATE FROM PERSON_INFO
	END
END

SELECT * FROM PERSON_INFO
INSERT INTO PERSON_INFO VALUES(103,'NEEL',10000,NULL,'RAJKOT',50,'2006-11-20')

-- 2.
CREATE TRIGGER PERSONLOG_MSG
ON PERSON_LOG
AFTER DELETE
AS
BEGIN
